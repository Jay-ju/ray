...
def _merge_columns(
    ...
    for block in stream:
        ...
        if tbl.num_columns > 1:
            print("###########33 can't be executed here")
    ...
    if len(commit_messages) == 0:        
        print(f"@@@@@@@@@@@@{tbl.to_pydict()['fragment_id']}")        
...
class LanceDatasink(_BaseLanceDatasink):
    ...
    def __init__(
        ...
        data_storage_version: Optional[str] = None,
        storage_options: Optional[Dict[str, Any]] = None,
        transform: Optional[Dict[str, str] | Callable[[pa.RecordBatch], pa.RecordBatch]] = None,
        *args,
        **kwargs,
    ):
        super().__init__(
        ...
        if mode not in SUPPORT_OPERATIONS:
            raise ValueError(f"Invalid mode: {mode}. Mode must be one of {SUPPORT_OPERATIONS}")
        self.mode = mode

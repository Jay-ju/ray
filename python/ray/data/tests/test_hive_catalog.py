...
from ray.data._internal.datasource.hive_catalog import HiveCatalog
from ray.data._internal.datasource.hive_catalog import HiveTypeSystem

@pytest.fixture(scope="module")
def hive_catalog():
    """Configure Hive Catalog test instance"""
    catalog = HiveCatalog(
        metastore_host="10.37.47.7",
        metastore_port=9083
    )
    yield catalog
...
def test_file_formats(hive_catalog, s3_filesystem, file_format, extension):
    ...
    hive_catalog.create_database(
        name="test_db",
        location=f"s3a://bucket1/test_db",
    ...
    dataset = hive_catalog.ray_read_dataset(
        ...
        **({
            ...
            # JSON parameters
            "read_json_args": {"orient": "records"} if file_format == "JSON" else {}
        })
    )
...
@pytest.mark.parametrize("file_format", ["PARQUET", "CSV", "JSON"])
def test_schema_conversion(hive_catalog, file_format):
